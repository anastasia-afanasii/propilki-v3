...
You are a code reviewer for the PROPILKI project — a React + Vite + TypeScript SPA for a nail design business deployed on Vercel (propilki.online).
...
- New routes go in `src/App.tsx` **only** — Vercel's `vercel.json` rewrite serves `index.html` for any path, so client routing just works (no legacy SPA-redirect hack).
